...
const VoidReasonManagementTab: React.FC = () => {
    ...
    const onSave = (reason: string, id?: string) => {
        handleSaveVoidReason({ id: id || `vr_${Date.now()}`, reason });
        closeModal();
    };

    const handleAddNew = () => {
        openModal('textInput', { title: 'Add Void Reason', label: 'Reason Text', onSubmit: (reason: string) => onSave(reason) });
    };

    const handleEdit = (reason: VoidReason) => {
        openModal('textInput', { title: 'Edit Void Reason', label: 'Reason Text', initialValue: reason.reason, onSubmit: (newReason: string) => onSave(newReason, reason.id) });
    };
    
    const handleDuplicate = (reason: VoidReason) => {
    ...
    return (
        <div className={cn("p-6 h-full flex flex-col", isFullScreen && "fixed inset-0 z-50 bg-card")}>
            <div className="flex justify-between items-center mb-4 no-print">
                <h3 className="text-xl font-bold text-foreground">{t('predefinedVoidReasons')}</h3>
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleAddNew} className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-2 px-3 rounded-lg text-sm">
                        <PlusIcon className="w-5 h-5" /> {t('addReason')}
                    </button>
            ...
            <div ref={tableContainerRef} className="overflow-auto bg-card rounded-lg flex-grow border border-border printable-area">
                <table className="min-w-full divide-y divide-border" style={{ tableLayout: 'fixed' }}>
                    ...
                    <thead className="bg-card sticky top-0 z-10 border-b border-border">
                        <tr>
                            <th className={`${thClass} relative`}>Reason <div onMouseDown={e => onMouseDown(0, e)} className="absolute top-0 right-0 w-1.5 h-full cursor-col-resize hover:bg-primary/50"/></th>
                    ...
                    <tbody className="bg-card divide-y divide-border">
                        {(voidReasons || []).map((reason: VoidReason) => (
                            <tr key={reason.id}>
                                <td className="px-4 py-3 text-foreground font-medium truncate">{reason.reason}</td>
                                <td className="px-4 py-3 text-end no-print">
                                    <div className="flex gap-2 justify-end">
                                        <button onClick={() => handleDuplicate(reason)} className="p-1 text-muted-foreground hover:text-foreground"><DocumentDuplicateIcon className="w-5 h-5"/></button>
                                        <button onClick={() => handleEdit(reason)} className="p-1 text-primary hover:opacity-80"><PencilSquareIcon className="w-5 h-5"/></button>
                                        <button onClick={() => handleDeleteVoidReason(reason.id)} className="p-1 text-destructive hover:opacity-80"><TrashIcon className="w-5 h-5"/></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
...
export default VoidReasonManagementTab;