...
const ModifierManagementTab: React.FC = () => {
    const { menuItems, handleDeleteModifierGroup, handleSaveModifierGroup } = useDataContext();
    const { modifierGroups, openModal, settings } = useAppContext();
    ...
    const handleAddNew = () => openModal('modifierGroupEdit');
    const handleEdit = (group: ModifierGroup) => openModal('modifierGroupEdit', { modifierGroup: group });
    const itemsUsingGroup = (groupId: string): number => (menuItems || []).filter((item: MenuItem) => item.modifierGroupIds?.includes(groupId)).length;
            ...
            <div className="flex justify-between items-center mb-4 gap-4 no-print">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleAddNew} className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-2 px-3 rounded-lg text-sm">
                        <PlusIcon className="w-5 h-5" /> {t('addNewModifierGroup')}
                    </button>
                        ...
                        {filteredModifierGroups.map((group: ModifierGroup) => (
                            <tr key={group.id}>
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-muted-foreground text-xs truncate">
                                    {group.allowMultiple && <span className="me-2">Multi-select</span>}
