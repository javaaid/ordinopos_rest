...
interface KIOSKViewProps {
    // onPlaceOrder prop is removed, will use broadcast channel instead
}
...
const KIOSKView: React.FC<KIOSKViewProps> = () => {
    ...
    useEffect(() => {
        if (step === 'confirmation') {
            const timer = setTimeout(() => {
                ...
                setStep('ordering'); // Go back to ordering, not welcome screen
    ...
    const renderWelcomeScreen = () => (
        <div className="flex flex-col items-center justify-center text-center relative w-full h-full">
            ...
            <footer className="absolute bottom-4 right-4 text-xs text-muted-foreground/50">
            ...
            <aside className="w-2/5 bg-card rounded-2xl flex flex-col p-6 shadow-2xl border border-border">
                ...
                {cart.length === 0 ? (
                    ...
                    <div className="flex-grow overflow-y-auto divide-y divide-border -mr-2 pr-2">
                        {cart.map(item => <OrderItem key={item.cartId} cartItem={item} onRemoveItem={handleRemoveItem} onUpdateCartQuantity={handleUpdateQuantity} orderType="kiosk" customer={null} />)}
                    </div>
                )}
    ...
    const renderPaymentScreen = () => (
         <div className="text-center bg-card/80 backdrop-blur-sm rounded-2xl p-20 shadow-2xl border border-border">
