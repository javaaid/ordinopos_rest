    ...
    return (
        <div className="bg-secondary h-9 px-2 rounded-lg flex items-center justify-between">
            <div className="flex items-center gap-2">
                <p className="text-sm font-semibold text-muted-foreground">{t('now_serving')}:</p>
                <p className="font-bold text-lg text-primary font-mono bg-background px-3 py-1 rounded-md">
                    {calledOrderNumber || '----'}
                </p>
            </div>
            <div className="flex items-center gap-1">
                <Button variant="ghost" size="sm" className="h-7" onClick={() => cycleCalledOrderNumber('prev')}>Prev</Button>
                <Button variant="ghost" size="sm" className="h-7" onClick={() => cycleCalledOrderNumber('next')}>Next</Button>
