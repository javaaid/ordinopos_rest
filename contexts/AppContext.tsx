...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
        ...
        setPlugins(prev => {
            return prev.map(p => {
                if (p.id === pluginId) {
                    ...
                    let newStatus: AppPlugin['status'] = 'disabled';
                    if (p.status === 'disabled') {
                        newStatus = p.isFree ? 'enabled' : (p.trialStartDate ? 'trial' : 'expired');
                    }
                    
                    addToast({ type: 'info', title: 'Plugin Status Changed', message: `${p.name} is now ${newStatus}.` });
    ...
    const handleActivatePlugin = useCallback((pluginId: string, code: string) => {
        if (ACTIVATION_CODES.has(code)) {
            setPlugins(prev => prev.map(p => {
                if (p.id === pluginId) {
                    addToast({ type: 'success', title: 'Plugin Activated', message: `${p.name} has been successfully activated.` });
                    return { ...p, status: 'enabled', activationDate: Date.now(), licenseDurationDays: 365 };
                }
                return p;
            }));
            closeModal();
        } else {
            addToast({ type: 'error', title: 'Activation Failed', message: 'The provided activation code is invalid.' });
        }
    }, [setPlugins, addToast, closeModal]);
    ...
    // FIX: Restore login functionality
    const handlePinLogin = useCallback((employeeId: string, pin: string): boolean => {
        ...
        return false;
    }, [employees, setCurrentEmployee, setAuditLog]);
    ...
    const contextValue = useMemo(() => ({
        ...
        currentLocation: locations.find((l: Location) => l.id === currentLocationId) || locations[0],
