...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [settings, setSettings] = usePersistentState<AppSettings>('settings', {
        ...
        receipt: { ...DEFAULT_RECEIPT_SETTINGS, template: 'standard', logoUrl: 'https://i.imgur.com/39g7c7s.png', promoMessage: 'Find us on social media @ordinopos!', footerLogoUrl: '' },
    ...
    const handleSendToKitchen = useCallback(() => {
        try {
            if (cart.length === 0) {
                addToast({ type: 'error', title: 'Empty Cart', message: 'Add items before sending to kitchen.' });
                return;
            }

            const currentLocation = locations.find(l => l.id === currentLocationId)!;
            const { subtotal, tax, total, taxDetails } = calculateOrderTotals(cart, currentLocation, null, null, 'dine-in', settings, selectedCustomer, surcharges);
            
            const newOrder: Order = {
                id: `ord_${Date.now()}`,
                orderNumber: 'K-' + String(settings.orderSettings.nextDailyOrderNumber).padStart(4, '0'),
                invoiceNumber: '',
                createdAt: Date.now(),
                cart: cart,
                employeeId: currentEmployee?.id,
            ...
            // FIX: Increment daily order number after using it.
            setSettings(prev => ({
                ...prev,
                orderSettings: {
                    ...prev.orderSettings,
            ...
            const jobsByPrinter = new Map<string, CartItem[]>();
            cart.forEach(item => {
                const printerId = item.menuItem.kitchenPrinterId || settings.devices.kitchenPrinterId;
                if (printerId) {
                    if (!jobsByPrinter.has(printerId)) jobsByPrinter.set(printerId, []);
                    jobsByPrinter.get(printerId)!.push(item);
                }
            });
            
            const printJobs: Omit<PrintJob, 'id' | 'timestamp' | 'status'>[] = [];
            jobsByPrinter.forEach((items, printerId) => {
                const printer = printers.find(p => p.id === printerId);
                if (!printer) return;
                const kitchenProfileKeys = Object.keys(printer.kitchenProfiles || {}) as KitchenProfileType[];
                const profileKey = kitchenProfileKeys[0]; 
                const profileName = profileKey ? KITCHEN_PROFILE_NAMES[profileKey] : 'Kitchen';
                 printJobs.push({
                    component: 'KitchenReceiptContent',
                    props: { order: { ...newOrder, cart: items }, settings: printer.kitchenProfiles?.[profileKey] || DEFAULT_KITCHEN_PRINT_SETTINGS, profileName }
                });
            });

            if (printJobs.length > 0) addPrintJobs(printJobs);
            if (currentTable) setTables(prev => prev.map(t => t.id === currentTable.id ? { ...t, status: 'occupied' as TableStatus, orderId: newOrder.id } : t));
            
            addToast({ type: 'success', title: 'Order Sent', message: 'Order has been sent to the kitchen.' });
            setCart([]); 

        } catch (error) {
            console.error("Error in handleSendToKitchen:", error);
            addToast({ type: 'error', title: 'Send Failed', message: 'Could not send order to the kitchen.' });
            addToast({ type: 'error', title: 'Kitchen Print Error', message: 'Failed to send order to kitchen. Please check printer connections.' });
        }
    }, [cart, addToast, locations, currentLocationId, settings, selectedCustomer, surcharges, currentEmployee, currentTable, setOrders, printers, addPrintJobs, setTables, setCart, setSettings]);
    ...
    const handleInitiatePayment = () => {
        try {
            const { subtotal, tax, total, taxDetails, finalAppliedDiscount, surchargeDetails, loyaltyDiscountAmount } = calculateOrderTotals(cart, locations.find(l => l.id === currentLocationId)!, appliedDiscount, appliedPromotion, orderType, settings, selectedCustomer, surcharges, appliedLoyaltyPoints);
            ...
            addToast({ type: 'error', title: 'Payment Error', message: 'Payment initiation failed.' });
        }
    };
        ...
        setTimeout(() => {
          try {
            if (activeTab) {
              ...
              setActiveTab(newTab);
              // FIX: Increment daily order number when a new tab is created.
              setSettings(prev => ({
                  ...prev,
                  orderSettings: {
                      ...prev.orderSettings,
    ...
    const contextValue = useMemo(() => ({
        ...
        onSelectItem: (item: MenuItem) => {
            if(item.modifierGroupIds && item.modifierGroupIds.length > 0) {
                openModal('modifier', { item, onAddItem: (itemWithMods: any, mods: any) => console.log(itemWithMods, mods), language: settings.language.staff });
            } else {
                setCart(prevCart => {
